...
func (f *Factorio) saveFileSetup() error {
	...
	if _, err := os.Stat(SaveFile); os.IsNotExist(err) {
		...
		gen := exec.Command(FactorioBinary, "--create", SaveFile)
		var output []byte
		...
		req := &api.SoftModRequest{Payload: payload, SkipRestart: true}
		if _, err = api.NewSoftModClient(f.Nats.conn).ApplySoftMod(req); err != nil {
			return err
		}
	}
	return nil
...
func (f *Factorio) Start(s *Service) error {
	...
	if err := s.Nats.Subscribe("factorio.stdin", f.stdinHandler); err != nil {
		return err
	}
	f.Infof("starting factorio")
	return f.cmd.Start()
}

func (f *Factorio) Restart() error {
	f.Infof("restarting factorio")
...
func (f *Factorio) Stop() error {
	if f.stdInPipe != nil {
		if err := f.stdInPipe.Close(); err != nil {
			f.Errorf("factorio stdin pipe close error: %v", err)
		}
		f.stdInPipe = nil
	}
	if f.cmd.Process != nil && f.cmd.ProcessState == nil {
		if err := f.cmd.Process.Signal(os.Interrupt); err != nil {
			f.Errorf("factorio interrupt error: %v", err)
			return err
		}
		if err := f.cmd.Wait(); err != nil {
			...
			f.Errorf("factorio wait error: %v", err)
			return err
		}
	}
	return nil
...
func (f *Factorio) publisher(pipeName string, pipe io.ReadCloser) {
	f.Infof("factorio.%s publisher entry %p", pipeName, pipe)
	defer func() {
		f.Infof("factorio.%s publisher exit %p", pipeName, pipe)
		_ = pipe.Close()
	}()
	...
	for scanner.Scan() {
		...
		line := scanner.Text()
		m.Data = []byte(line)
		f.Infof("factorio.%s %s", pipeName, line)
		f.Nats.PublishMsg(m)
	}
	if err := scanner.Err(); err != nil {
		if !strings.Contains(err.Error(), "file already closed") {
			f.Errorf("factorio.%s scanner error: %v", pipeName, err)
		}
	}
}
...
func (f *Factorio) stdinHandler(msg *nats.Msg) {
	if f.stdInPipe != nil {
		...
		if !strings.HasPrefix(logMessage, "/silent-command") {
			f.Infof("factorio.stdin %s", logMessage)
		}
